...
// ServerScanner 服务器扫描服务
type ServerScanner struct {
	// 服务器信息统计服务（mod/插件/类型/版本）
	info *ServerInfo
}

// NewServerScanner 创建扫描服务实例
func NewServerScanner() *ServerScanner {
...
// 返回服务器实例列表
func (s *ServerScanner) ScanServers() (*model.ServerListResult, error) {
	// 1. 获取 exe 所在目录
	...
	for _, entry := range entries {
		...
		if instance.HasJar {
			...
			// 通过 ServerInfo 查询 mod/插件/类型/版本统计信息
			serverConfigs = append(serverConfigs, model.ServerConfig{
				ServerName: serverName,
				Info:       s.info.ScanServerInfo(serverFolderPath),
			})
		}
	}
...
func (s *ServerScanner) persistServerConfigs(configs []model.ServerConfig) {
	cfg := config.NewConfigService()

	// 读取现有配置
	existing, err := cfg.LoadServerList()
	if err != nil {
		existing = []model.ServerConfig{}
	}
...
func (s *ServerScanner) scanServerFolder(serverName, serverFolderPath string) model.ServerInstance {
	// 初始化服务器实例，记录服务器名与路径
...
func (s *ServerScanner) checkEulaAgreed(eulaPath string) bool {
...
func (s *ServerScanner) GetServerPath() (string, error) {
