...
// GetJavaListFromConfig 从 config 读取已保存的 Java 列表
func (s *JavaService) GetJavaListFromConfig() []model.JavaInfo {
...
// SaveJavaListToConfig 保存 Java 列表到 config
func (s *JavaService) SaveJavaListToConfig(list []model.JavaInfo) error {
	path := s.getConfigPath()
...
// getConfigPath 获取 config/JavaList.json 路径
// 统一由 utils.GetConfigDir 提供 config 目录，避免重复拼接
func (s *JavaService) getConfigPath() string {
	// 在统一 config 目录下拼接 JavaList.json
	return filepath.Join(utils.GetConfigDir(), "JavaList.json")
}
