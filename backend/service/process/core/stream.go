...
func (s *ProcessManager) streamLines(reader io.Reader) {
	...
	for {
		...
		if len(line) > 0 {
			...
			runtime.EventsEmit(s.ctx, "server:log", cleanLine)
			// 交给检测器分析（类型匹配 + 版本静默检测）
			s.handleDetectionLine(cleanLine)
		}
		if err != nil {
